...
@functools.cache
def _get_token_from_local_storage():
    """Read FPT Play auth token from Firefox localStorage."""
    profile_root = os.path.expanduser(
        r'~/AppData/Roaming/Mozilla/Firefox/Profiles')
    if not os.path.isdir(profile_root):
        return None
    for profile in glob.glob(os.path.join(profile_root, '*')):
        db_path = os.path.join(
            profile, 'storage', 'default',
            'https+++fptplay.vn', 'ls', 'data.sqlite')
        if not os.path.isfile(db_path):
            continue
        try:
            db = f'file:{db_path}?mode=ro&immutable=1'
            conn = sqlite3.connect(db, uri=True)
            cur = conn.execute(
                "SELECT value FROM data WHERE key = 'token'")
            row = cur.fetchone()
            conn.close()
            if row and row[0]:
                # Token is stored as raw JWT string (blob)
                token = row[0]
...
class FptplayIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        # Token may be in cookies or Firefox localStorage
        token = self._get_cookies(url).get("token")
        if not token:
            token = _get_token_from_local_storage()
        if not token:
            raise ExtractorError(
                'Missing authentication token. '
                'Please login to FPTPlay first in Firefox.', expected=True)

        # If token came from localStorage, wrap it to mimic cookie behavior
        ...
        if res["result"]["episode_type"] == 0:
            ...
            manifest = self._download_json(self.get_api_with_st_token(contentId, 0), contentId, headers={'authorization': f'Bearer {token_value}'}, expected_status=406)
            ...
            # playlist
            entries = []
            for idx, episode in enumerate(res["result"]["episodes"]):
                
                if episode["is_trailer"] == 1:
                    continue
                
                manifest = self._download_json(self.get_api_with_st_token(contentId, episode["_id"]), episode["_id"], headers={'authorization': f'Bearer {token_value}'}, expected_status=406)
                if manifest.get("msg") != "success":
                    raise ExtractorError(f' - Got an error, response: {manifest.get("msg")}', expected=True)
    ...
    def get_api_with_st_token(self, video_id, episode=None):
        if episode is not None:
            path = f'/api/v7.1_w/stream/vod/{video_id}/{episode}/adaptive_bitrate'
        else:
